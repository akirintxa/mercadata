...
- En Farmatodo (`searchFarmatodo`), la disponibilidad (`inStock`) se determina por `stores_with_stock.length > 0` (tiendas físicas con inventario real), no por el campo `hasStock` de su API — ese campo refleja un almacén/tienda de referencia puntual y es `false` incluso para productos activamente vendidos en cientos de tiendas, lo que marcaba como "agotados" artículos de consumo de hogar de alta rotación que sí están disponibles.
