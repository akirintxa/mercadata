...
- `POST /api/compare-list` recibe `{ items: string[] }` y, para cada producto, reutiliza el mismo pipeline de búsqueda y emparejamiento (`searchAllStores` / `evaluateProductMatch`) que `/api/search`, tomando el resultado más barato en stock por tienda.
